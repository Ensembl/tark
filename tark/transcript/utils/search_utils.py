...
class SearchUtils(object):
    ...
    def parse_hgvs_genomic_location_string(cls, hgvs_string):
        ...
        loc_region, loc_assemby = cls.get_seq_region_from_refseq_accession(
            refseq_accession
        )
        loc_start = matchloc.group(2)
        loc_end = loc_start
    ...
    def get_lrg_id_from_hgnc_name(cls, hgnc_name):
        try:
            host_url_root = "https://www.ebi.ac.uk/ebisearch/ws/rest/lrg?query=name:{}&format=json".format(
                hgnc_name
            )

            response = requests.get(host_url_root, timeout=5)
            if response.status_code == 200:
                ...
                if (
                    "hitCount" in search_result_dict
                    and search_result_dict["hitCount"] == 1
                ):
            ...
            logger.warning("LRG lookup failed for %s", hgnc_name)
            return None
